...
export class DashboardService implements IDashboardService {
  ...
  getStartAndEndDate(date?: DashboardDto): DashboardSerialization {
    ...
    if (!startDate) startDate = this.helperDateService.startOfMonth();
    else startDate = this.helperDateService.startOfDay(startDate);

    if (!endDate) endDate = this.helperDateService.endOfMonth();
    else endDate = this.helperDateService.endOfDay(endDate);
