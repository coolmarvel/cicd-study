...
import { AuthJwtAccessProtected, AuthJwtPayload, AuthJwtRefreshProtected, AuthJwtToken } from 'src/common/auth/decorators/auth.jwt.decorator';
...
export class UserAuthController {
  ...
  async login(@Body() { email, password }: UserLoginDto): Promise<IResponse> {
    ...
    const payloadAccessToken: AuthAccessPayloadSerialization = await this.authService.createPayloadAccessToken(payload, {
      loginWith: ENUM_AUTH_LOGIN_WITH.EMAIL,
      loginFrom: ENUM_AUTH_LOGIN_FROM.PASSWORD,
      loginDate,
    });
    const payloadRefreshToken: AuthRefreshPayloadSerialization = await this.authService.createPayloadRefreshToken(payload._id, payloadAccessToken);
    ...
    return { data: { tokenType, roleType, expiresIn, accessToken, refreshToken } };
  }
  ...
  @ApiKeyPublicProtected()
  @Get('/login/google')
  async loginGoogle(@AuthJwtPayload<AuthGooglePayloadSerialization>() { user: userPayload }: AuthGooglePayloadSerialization): Promise<IResponse> {
  ...
  async refresh(
    ...
    @AuthJwtPayload<AuthRefreshPayloadSerialization>() refreshPayload: AuthRefreshPayloadSerialization,
    ...
    if (payloadEncryption) payloadHashedAccessToken = await this.authService.encryptAccessToken(payloadAccessToken);
  ...
  async changePassword(@Body() body: UserChangePasswordDto, @GetUser() user: UserDoc): Promise<void> {
    const passwordAttempt: boolean = await this.authService.getPasswordAttempt();
    const maxPasswordAttempt: number = await this.authService.getMaxPasswordAttempt();
    if (passwordAttempt && user.passwordAttempt >= maxPasswordAttempt) {
      throw new ForbiddenException({
    ...
    try {
      ...
      const password: IAuthPassword = await this.authService.createPassword(body.newPassword);
  ...
  @ApiKeyPublicProtected()
  @Get('/info')
  async info(@AuthJwtPayload<AuthAccessPayloadSerialization>() payload: AuthAccessPayloadSerialization): Promise<IResponse> {
    return { data: payload };
  }
  ...
  async upload(
    @GetUser() user: UserDoc,
    @UploadedFile(new FileRequiredPipe(), new FileTypePipe([ENUM_FILE_MIME.PNG, ENUM_FILE_MIME.JPG, ENUM_FILE_MIME.JPEG]))
    file: IFile,
  ): Promise<void> {
