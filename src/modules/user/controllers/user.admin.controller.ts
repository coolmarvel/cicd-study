import { Controller, Get, Post, Body, Delete, Put, NotFoundException, UploadedFile, ConflictException, Patch, HttpCode, HttpStatus } from '@nestjs/common';
...
import { Response, ResponseFileExcel, ResponsePaging } from 'src/common/response/decorators/response.decorator';
import { IResponse, IResponseFileExcel, IResponsePaging } from 'src/common/response/interfaces/response.interface';
...
import { PaginationQuery, PaginationQueryFilterEqualObjectId, PaginationQueryFilterInBoolean } from 'src/common/pagination/decorators/pagination.decorator';
...
export class UserAdminController {
  ...
  @Get('/list')
  async list(
    @PaginationQuery(USER_DEFAULT_PER_PAGE, USER_DEFAULT_ORDER_BY, USER_DEFAULT_ORDER_DIRECTION, USER_DEFAULT_AVAILABLE_SEARCH, USER_DEFAULT_AVAILABLE_ORDER_BY)
    ...
    const find: Record<string, any> = { ..._search, ...isActive, ...blocked, ...inactivePermanent, ...role };

    const users: IUserEntity[] = await this.userService.findAll<IUserEntity>(find, {
    ...
    return { _pagination: { total, totalPage }, data: users };
  }
  ...
  @ApiKeyPublicProtected()
  @Post('/create')
  async create(@Body() { email, mobileNumber, role, ...body }: UserCreateDto): Promise<IResponse> {
    const promises: Promise<any>[] = [this.roleService.findOneById(role), this.userService.existByEmail(email)];

    if (mobileNumber) promises.push(this.userService.existByMobileNumber(mobileNumber));
    ...
    const created: UserDoc = await this.userService.create({ email, mobileNumber, signUpFrom: ENUM_USER_SIGN_UP_FROM.ADMIN, role, ...body }, password);
  ...
  @Post('/import')
  async import(
    @UploadedFile(new FileRequiredPipe(), new FileTypePipe([ENUM_FILE_MIME.CSV]), FileExcelExtractPipe, new FileExcelValidationPipe<UserImportDto>(UserImportDto))
    ...
    const mobileNumbers = file.extracts[0].data.map((e) => e.mobileNumber).filter((e) => e) ?? [];
    const emails = file.extracts[0].data.map((e) => e.email) ?? [];

    const promises: Promise<any>[] = [this.userService.existByEmails(emails)];

    if (mobileNumbers && mobileNumbers.length > 0) promises.push(this.userService.existByMobileNumbers(mobileNumbers));
    ...
    const passwordString: string = await this.authService.createPasswordRandom();
    const password: IAuthPassword = await this.authService.createPassword(passwordString);

    await this.userService.import(file.extracts[0].data, checkRole._id, password);
  ...
  async export(): Promise<IResponseFileExcel> {
    const users: IUserEntity[] = await this.userService.findAll<IUserEntity>({}, { plainObject: true });
