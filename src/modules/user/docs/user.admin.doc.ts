...
import { Doc, DocAuth, DocRequest, DocRequestFile, DocGuard, DocResponse, DocResponseFile, DocResponsePaging } from 'src/common/doc/decorators/doc.decorator';
import { FileSingleDto } from 'src/common/file/dtos/file.single.dto';
import { ResponseIdSerialization } from 'src/common/response/serializations/response.id.serialization';
import { UserDocParamsId, UserDocQueryBlocked, UserDocQueryInactivePermanent, UserDocQueryIsActive, UserDocQueryRole } from 'src/modules/user/constants/user.doc.constant';
...
export function UserAdminListDoc(): MethodDecorator {
  return applyDecorators(
    ...
    DocRequest({ queries: [...UserDocQueryIsActive, ...UserDocQueryBlocked, ...UserDocQueryInactivePermanent, ...UserDocQueryRole] }),
    DocAuth({ apiKey: true, jwtAccessToken: true }),
    DocGuard({ role: true, policy: true }),
    DocResponsePaging<UserListSerialization>('user.list', { serialization: UserListSerialization }),
  );
}
...
export function UserAdminGetDoc(): MethodDecorator {
  return applyDecorators(
    ...
    DocAuth({ apiKey: true, jwtAccessToken: true }),
    DocGuard({ role: true, policy: true }),
    DocResponse<UserGetSerialization>('user.get', { serialization: UserGetSerialization }),
  );
}

export function UserAdminActiveDoc(): MethodDecorator {
  return applyDecorators(
    Doc({ summary: 'make user be active' }),
    DocRequest({ params: UserDocParamsId }),
    DocAuth({ apiKey: true, jwtAccessToken: true }),
    DocGuard({ role: true, policy: true }),
...
export function UserAdminInactiveDoc(): MethodDecorator {
  return applyDecorators(
    Doc({ summary: 'make user be inactive' }),
    DocRequest({ params: UserDocParamsId }),
    DocAuth({ apiKey: true, jwtAccessToken: true }),
    DocGuard({ role: true, policy: true }),
...
export function UserAdminUpdateDoc(): MethodDecorator {
  return applyDecorators(
    Doc({ summary: 'update data a user' }),
    DocRequest({
...
export function UserAdminImportDoc(): MethodDecorator {
  return applyDecorators(
    Doc({ summary: 'import users with excel' }),
    DocAuth({ apiKey: true, jwtAccessToken: true }),
...
export function UserAdminExportDoc(): MethodDecorator {
  return applyDecorators(Doc({ summary: 'export user into excel' }), DocAuth({ apiKey: true, jwtAccessToken: true }), DocGuard({ role: true, policy: true }), DocResponseFile());
}
