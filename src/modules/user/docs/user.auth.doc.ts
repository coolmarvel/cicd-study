...
export function UserAuthLoginDoc(): MethodDecorator {
  return applyDecorators(
    Doc({ summary: 'login with email and password' }),
    DocRequest({
...
export function UserAuthLoginGoogleDoc(): MethodDecorator {
  return applyDecorators(Doc({ summary: 'login with access token google' }), DocAuth({ google: true, apiKey: true }), DocResponse('user.loginGoogle'));
}
...
export function UserAuthProfileDoc(): MethodDecorator {
  return applyDecorators(
    ...
    DocResponse<UserProfileSerialization>('user.profile', { serialization: UserProfileSerialization }),
  );
}

export function UserAuthUploadProfileDoc(): MethodDecorator {
  return applyDecorators(
    Doc({ summary: 'update profile photo' }),
    DocAuth({ apiKey: true, jwtAccessToken: true }),
...
export function UserAuthInfoDoc(): MethodDecorator {
  return applyDecorators(
    Doc({ summary: 'get info of access token' }),
    DocAuth({ apiKey: true, jwtAccessToken: true }),
    DocResponse<AuthAccessPayloadSerialization>('user.info', { serialization: AuthAccessPayloadSerialization }),
  );
}
