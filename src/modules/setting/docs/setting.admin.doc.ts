...
import { Doc, DocAuth, DocDefault, DocErrorGroup, DocRequest, DocGuard, DocResponse, DocResponsePaging } from 'src/common/doc/decorators/doc.decorator';
...
export function SettingAdminListDoc(): MethodDecorator {
  return applyDecorators(
    Doc({ summary: 'get list of settings' }),
    DocAuth({ apiKey: true }),
    DocResponsePaging<SettingListSerialization>('setting.list', { serialization: SettingListSerialization }),
  );
}
...
export function SettingAdminGetDoc(): MethodDecorator {
  return applyDecorators(
    ...
    DocResponse<SettingGetSerialization>('setting.get', { serialization: SettingGetSerialization }),
    DocAuth({ apiKey: true }),
    DocErrorGroup([
      DocDefault({
...
export function SettingAdminUpdateDoc(): MethodDecorator {
  return applyDecorators(
    ...
    DocAuth({ jwtAccessToken: true, apiKey: true }),
    DocResponse<ResponseIdSerialization>('setting.update', { serialization: ResponseIdSerialization }),
