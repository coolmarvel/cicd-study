...
import { SettingDoc, SettingEntity } from 'src/modules/setting/repository/entities/setting.entity';
...
export class SettingService implements ISettingService {
  ...
  constructor(
    ...
    this.timezoneOffset = this.helperDateService.format(this.helperDateService.create(), { format: ENUM_HELPER_DATE_FORMAT.TIMEZONE });
  }

  async findAll<T = SettingDoc>(find?: Record<string, any>, options?: IDatabaseFindAllOptions): Promise<T[]> {
    return this.settingRepository.findAll<T>(find, options);
  }

  async findOneById(_id: string, options?: IDatabaseFindOneOptions): Promise<SettingDoc> {
    return this.settingRepository.findOneById<SettingDoc>(_id, options);
  }

  async findOneByName(name: string, options?: IDatabaseFindOneOptions): Promise<SettingDoc> {
  ...
  async create({ name, description, type, value }: SettingCreateDto, options?: IDatabaseCreateOptions): Promise<SettingDoc> {
  ...
  async updateValue(repository: SettingDoc, { type, value }: SettingUpdateValueDto, options?: IDatabaseSaveOptions): Promise<SettingDoc> {
  ...
  async delete(repository: SettingDoc, options?: IDatabaseSaveOptions): Promise<SettingDoc> {
    return this.settingRepository.softDelete(repository, options);
  }

  async getValue<T>(setting: SettingDoc): Promise<T> {
    if (setting.type === ENUM_SETTING_DATA_TYPE.BOOLEAN && (setting.value === 'true' || setting.value === 'false')) return (setting.value === 'true') as any;
    else if (setting.type === ENUM_SETTING_DATA_TYPE.NUMBER && this.helperNumberService.check(setting.value)) return Number.parseInt(setting.value) as any;
    else if (setting.type === ENUM_SETTING_DATA_TYPE.ARRAY_OF_STRING) return setting.value.split(',') as any;

    return setting.value as any;
  }

  async checkValue(value: string, type: ENUM_SETTING_DATA_TYPE): Promise<boolean> {
    if (type === ENUM_SETTING_DATA_TYPE.BOOLEAN && (value === 'true' || value === 'false')) return true;
    else if (type === ENUM_SETTING_DATA_TYPE.NUMBER && this.helperNumberService.check(value)) return true;
    else if ((type === ENUM_SETTING_DATA_TYPE.STRING || type === ENUM_SETTING_DATA_TYPE.ARRAY_OF_STRING) && typeof value === 'string') return true;
