...
import { SettingAdminGetGuard, SettingAdminUpdateGuard } from 'src/modules/setting/decorators/setting.admin.decorator';
import { GetSetting } from 'src/modules/setting/decorators/setting.decorator';
import { SettingAdminGetDoc, SettingAdminListDoc, SettingAdminUpdateDoc } from 'src/modules/setting/docs/setting.admin.doc';
...
export class SettingAdminController {
  ...
  @Get('/list')
  async list(
    @PaginationQuery(SETTING_DEFAULT_PER_PAGE, SETTING_DEFAULT_ORDER_BY, SETTING_DEFAULT_ORDER_DIRECTION, SETTING_DEFAULT_AVAILABLE_SEARCH, SETTING_DEFAULT_AVAILABLE_ORDER_BY)
    { _search, _limit, _offset, _order }: PaginationListDto,
  ): Promise<IResponsePaging> {
    const find: Record<string, any> = { ..._search };

    const settings: SettingEntity[] = await this.settingService.findAll<SettingEntity>(find, {
    ...
    return { _pagination: { total, totalPage }, data: settings };
  }
  ...
  @RequestParamGuard(SettingRequestDto)
  @Put('/update/:setting')
  async update(@GetSetting() setting: SettingDoc, @Body() body: SettingUpdateValueDto): Promise<IResponse> {
