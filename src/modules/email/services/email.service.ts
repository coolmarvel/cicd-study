...
import { EMAIL_CHANGE_PASSWORD_HTML_BODY, EMAIL_SIGN_UP_HTML_BODY } from 'src/modules/email/constants/email.constant';
...
export class EmailService implements IEmailService {
  ...
  async getSignUp(): Promise<boolean> {
    try {
      await this.awsSESService.getTemplate({ name: ENUM_EMAIL.SIGN_UP });
  ...
  async deleteSignUp(): Promise<boolean> {
    try {
      await this.awsSESService.deleteTemplate({ name: ENUM_EMAIL.SIGN_UP });
  ...
  async sendSignUp(user: UserDoc): Promise<boolean> {
    try {
      await this.awsSESService.send<EmailSendSignUpDto>({
        ...
        templateData: { appName: capital(this.appName), name: capital(`${user.firstName} ${user.lastName}`) },
      });
  ...
  async createChangePassword(): Promise<boolean> {
    try {
      await this.awsSESService.createTemplate({
        ...
        subject: `${this.appName} ${capital(ENUM_EMAIL.CHANGE_PASSWORD)}`,
  ...
  async getChangePassword(): Promise<boolean> {
    try {
      await this.awsSESService.getTemplate({ name: ENUM_EMAIL.CHANGE_PASSWORD });
  ...
  async deleteChangePassword(): Promise<boolean> {
    try {
      await this.awsSESService.deleteTemplate({ name: ENUM_EMAIL.CHANGE_PASSWORD });
  ...
  async sendChangePassword(user: UserDoc): Promise<boolean> {
    try {
      await this.awsSESService.send<EmailSendChangePasswordDto>({
        ...
        recipients: [user.email],
        sender: this.fromEmail,
        templateData: { name: capital(`${user.firstName} ${user.lastName}`) },
      });
