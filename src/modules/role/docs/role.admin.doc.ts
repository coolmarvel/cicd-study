...
import { Doc, DocAuth, DocRequest, DocGuard, DocResponse, DocResponsePaging } from 'src/common/doc/decorators/doc.decorator';
import { ResponseIdSerialization } from 'src/common/response/serializations/response.id.serialization';
import { RoleDocParamsId, RoleDocQueryIsActive, RoleDocQueryType } from 'src/modules/role/constants/role.doc.constant';
...
export function RoleAdminListDoc(): MethodDecorator {
  return applyDecorators(
    Doc({ summary: 'get all of roles' }),
    DocRequest({ queries: [...RoleDocQueryIsActive, ...RoleDocQueryType] }),
    DocAuth({ apiKey: true, jwtAccessToken: true }),
    DocGuard({ role: true, policy: true }),
    DocResponsePaging<RoleListSerialization>('role.list', { serialization: RoleListSerialization }),
  );
}
...
export function RoleAdminGetDoc(): MethodDecorator {
  return applyDecorators(
    ...
    DocAuth({ apiKey: true, jwtAccessToken: true }),
    DocGuard({ role: true, policy: true }),
    DocResponse<RoleGetSerialization>('role.get', { serialization: RoleGetSerialization }),
  );
}
...
export function RoleAdminActiveDoc(): MethodDecorator {
  return applyDecorators(
    Doc({ summary: 'make role be active' }),
    DocRequest({ params: RoleDocParamsId }),
    DocAuth({ apiKey: true, jwtAccessToken: true }),
    DocGuard({ role: true, policy: true }),
...
export function RoleAdminInactiveDoc(): MethodDecorator {
  return applyDecorators(
    Doc({ summary: 'make role be inactive' }),
    DocRequest({ params: RoleDocParamsId }),
    DocAuth({ apiKey: true, jwtAccessToken: true }),
    DocGuard({ role: true, policy: true }),
...
export function RoleAdminUpdateDoc(): MethodDecorator {
  return applyDecorators(
    ...
    DocAuth({ apiKey: true, jwtAccessToken: true }),
    DocGuard({ role: true, policy: true }),
    DocResponse<ResponseIdSerialization>('role.update', { serialization: ResponseIdSerialization }),
  );
}
