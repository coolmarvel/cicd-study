...
export class RolePayloadTypeGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    const requiredFor: ENUM_ROLE_TYPE[] = this.reflector.getAllAndOverride<ENUM_ROLE_TYPE[]>(ROLE_TYPE_META_KEY, [context.getHandler(), context.getClass()]);

    const { user } = context.switchToHttp().getRequest<IRequestApp>();
    const { type } = user.user;

    if (!requiredFor || type === ENUM_ROLE_TYPE.SUPER_ADMIN) return true;
