...
import { PaginationQuery, PaginationQueryFilterInBoolean, PaginationQueryFilterInEnum } from 'src/common/pagination/decorators/pagination.decorator';
...
export class RoleAdminController {
  ...
  @Get('/list')
  async list(
    @PaginationQuery(ROLE_DEFAULT_PER_PAGE, ROLE_DEFAULT_ORDER_BY, ROLE_DEFAULT_ORDER_DIRECTION, ROLE_DEFAULT_AVAILABLE_SEARCH, ROLE_DEFAULT_AVAILABLE_ORDER_BY)
    ...
    const find: Record<string, any> = { ..._search, ...isActive, ...type };

    const roles: RoleEntity[] = await this.roleService.findAll<RoleEntity>(find, {
  ...
  @ApiKeyPublicProtected()
  @Post('/create')
  async create(@Body() { name, description, type, permissions }: RoleCreateDto): Promise<IResponse> {
    ...
    const create = await this.roleService.create({ name, description, type, permissions });

    return { data: { _id: create._id } };
  }

  @RoleAdminUpdateDoc()
  @Response('role.update', { serialization: ResponseIdSerialization })
  @RoleAdminUpdateGuard()
  @PolicyAbilityProtected({
    subject: ENUM_POLICY_SUBJECT.ROLE,
  ...
  @RequestParamGuard(RoleRequestDto)
  @Put('/update/:role')
  async update(@GetRole() role: RoleDoc, @Body() { description }: RoleUpdateDto): Promise<IResponse> {
    await this.roleService.update(role, { description });

    return { data: { _id: role._id } };
  }

  @RoleAdminUpdateDoc()
  @Response('role.updatePermission', { serialization: ResponseIdSerialization })
  @RoleAdminUpdateGuard()
  @PolicyAbilityProtected({
    subject: ENUM_POLICY_SUBJECT.ROLE,
    action: [ENUM_POLICY_ACTION.READ, ENUM_POLICY_ACTION.UPDATE],
  ...
  async updatePermission(@GetRole() role: RoleDoc, @Body() { permissions, type }: RoleUpdatePermissionDto): Promise<IResponse> {
    await this.roleService.updatePermissions(role, { permissions, type });
  ...
  async delete(@GetRole() role: RoleDoc): Promise<void> {
    const used: UserDoc = await this.userService.findOne({ role: role._id });
    if (used) {
      throw new ConflictException({
