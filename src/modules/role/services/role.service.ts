...
export class RoleService implements IRoleService {
  ...
  async findAll<T = RoleDoc>(find?: Record<string, any>, options?: IDatabaseFindAllOptions): Promise<T[]> {
  ...
  async create({ name, description, type, permissions }: RoleCreateDto, options?: IDatabaseCreateOptions): Promise<RoleDoc> {
  ...
  async update(repository: RoleDoc, { description }: RoleUpdateDto, options?: IDatabaseSaveOptions): Promise<RoleDoc> {
    repository.description = description;

    return this.roleRepository.save(repository, options);
  }

  async updatePermissions(repository: RoleDoc, { permissions, type }: RoleUpdatePermissionDto, options?: IDatabaseSaveOptions): Promise<RoleDoc> {
  ...
  async createMany(data: RoleCreateDto[], options?: IDatabaseCreateManyOptions): Promise<boolean> {
