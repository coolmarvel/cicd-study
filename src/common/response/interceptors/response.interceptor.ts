...
import { ResponseSerialization, ResponseMetadataSerialization } from 'src/common/response/serializations/response.serialization';
import {
...
export class ResponseInterceptor<T> implements NestInterceptor<Promise<T>> {
  ...
  async intercept(context: ExecutionContext, next: CallHandler): Promise<Observable<Promise<ResponseSerialization>>> {
    if (context.getType() === 'http') {
      return next.handle().pipe(
        map(async (res: Promise<Record<string, any>>) => {
          ...
          const message: string | IMessage = await this.messageService.get(messagePath, { customLanguages: __customLang, properties: messageProperties });
