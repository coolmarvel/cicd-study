...
export class ResponseFileExcelInterceptor implements NestInterceptor<Promise<any>> {
  ...
  async intercept(context: ExecutionContext, next: CallHandler): Promise<Observable<Promise<Record<string, any>>>> {
    if (context.getType() === 'http') {
      return next.handle().pipe(
        map(async (res: Promise<IResponseFileExcel>) => {
          ...
          const password: string = this.reflector.get<string>(RESPONSE_FILE_EXCEL_PASSWORD_META_KEY, context.getHandler());

          const classSerialization: ClassConstructor<any>[] = this.reflector.get<ClassConstructor<any>[]>(RESPONSE_FILE_EXCEL_SERIALIZATION_META_KEY, context.getHandler());
          ...
          if (type === ENUM_HELPER_FILE_EXCEL_TYPE.XLSX) {
            ...
            if (classSerialization && Array.isArray(classSerialization) && classSerialization.length > 0) {
              for (const [index, data] of datas.entries()) {
                if (classSerialization[index]) {
                  const dataSerialization = plainToInstance(classSerialization[index], data, classSerializationOptions);
            ...
            // create file
            const file: Buffer = this.helperFileService.writeExcel(datas, { password });
          ...
          if (classSerialization && Array.isArray(classSerialization) && classSerialization.length > 0) {
            if (classSerialization[0]) {
              const dataSerialization = plainToInstance(classSerialization[0], data.data, classSerializationOptions);
