...
export class ResponsePagingInterceptor<T> implements NestInterceptor<Promise<T>> {
  ...
  async intercept(context: ExecutionContext, next: CallHandler): Promise<Observable<Promise<ResponsePagingSerialization>>> {
    if (context.getType() === 'http') {
      return next.handle().pipe(
        map(async (res: Promise<IResponsePaging>) => {
          ...
          const classSerialization: ClassConstructor<any> = this.reflector.get<ClassConstructor<any>>(RESPONSE_SERIALIZATION_META_KEY, context.getHandler());
          const classSerializationOptions: ClassTransformOptions = this.reflector.get<ClassTransformOptions>(RESPONSE_SERIALIZATION_OPTIONS_META_KEY, context.getHandler());
          let messageProperties: IMessageOptionsProperties = this.reflector.get<IMessageOptionsProperties>(RESPONSE_MESSAGE_PROPERTIES_META_KEY, context.getHandler());
          ...
          const cursorPaginationMetadata: ResponsePaginationCursorSerialization = {
            nextPage:
              page < totalPage ? (queryString ? `${__path}?perPage=${perPage}&page=${page + 1}&${queryString}` : `${__path}?perPage=${perPage}&page=${page + 1}`) : undefined,
            previousPage: page > 1 ? (queryString ? `${__path}?perPage=${perPage}&page=${page - 1}&${queryString}` : `${__path}?perPage=${perPage}&page=${page - 1}`) : undefined,
            firstPage: totalPage > 1 ? (queryString ? `${__path}?perPage=${perPage}&page=${1}&${queryString}` : `${__path}?perPage=${perPage}&page=${1}`) : undefined,
            lastPage:
              totalPage > 1 ? (queryString ? `${__path}?perPage=${perPage}&page=${totalPage}&${queryString}` : `${__path}?perPage=${perPage}&page=${totalPage}`) : undefined,
          };

          metadata = {
            ...metadata,
            ..._metadata,
            pagination: { ...__pagination, ...metadata._pagination, total, totalPage: data.length > 0 ? totalPage : 0 },
          };
          ...
          return { statusCode, message, _metadata: metadata, data };
        }),
      );
    }
