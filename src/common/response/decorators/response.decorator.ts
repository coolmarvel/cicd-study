...
import { IResponseOptions, IResponsePagingOptions, IResponseFileExcelOptions } from 'src/common/response/interfaces/response.interface';
...
export function ResponseFileExcel<T>(options?: IResponseFileExcelOptions<T>): MethodDecorator {
  return applyDecorators(
    UseInterceptors(ResponseFileExcelInterceptor),
    SetMetadata(RESPONSE_FILE_EXCEL_SERIALIZATION_META_KEY, options?.serialization),
...
export function ResponsePaging<T>(messagePath: string, options: IResponsePagingOptions<T>): MethodDecorator {
  return applyDecorators(
