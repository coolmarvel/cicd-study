import { ExceptionFilter, Catch, ArgumentsHost, HttpException, HttpStatus, Optional, InternalServerErrorException } from '@nestjs/common';
...
import { IErrorException, IErrorValidationImport, IErrors, IErrorsImport } from 'src/common/error/interfaces/error.interface';
...
export class ErrorHttpFilter implements ExceptionFilter {
  ...
  async catch(exception: unknown, host: ArgumentsHost): Promise<void> {
    ...
    const __timestamp = request.__xTimestamp ?? request.__timestamp ?? this.helperDateService.createTimestamp();
        ...
        if (responseException.errors?.length > 0) {
          errors =
            responseException._errorType === ERROR_TYPE.IMPORT
              ? this.messageService.getImportErrorsMessage(responseException.errors as IErrorValidationImport[], { customLanguages: __customLang })
              : this.messageService.getRequestErrorsMessage(responseException.errors as ValidationError[], { customLanguages: __customLang });
        }

        if (!responseException._error) {
          _error = typeof responseException._error !== 'string' ? JSON.stringify(responseException._error) : responseException._error;
        }
      }
    }
    ...
    const responseBody = { statusCode, message, errors, _error, _metadata: metadata, data };
