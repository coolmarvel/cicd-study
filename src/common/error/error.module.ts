...
@Module({
  controllers: [],
  providers: [
    { provide: APP_FILTER, useClass: ErrorHttpFilter },
    { provide: APP_GUARD, useClass: ErrorMetaGuard },
  ],
  imports: [
    SentryModule.forRootAsync({
      ...
      useFactory: async (configService: ConfigService) => ({
        ...
        logLevels: configService.get<LogLevel[]>('debugger.sentry.logLevel'),
        close: { enabled: true, timeout: configService.get<number>('debugger.sentry.timeout') },
      }),
