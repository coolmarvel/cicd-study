...
export class ApiKeyActiveGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    const required: boolean[] = this.reflector.getAllAndOverride<boolean[]>(API_KEY_ACTIVE_META_KEY, [context.getHandler(), context.getClass()]);

    if (!required) return true;

    const { __apiKey } = context.switchToHttp().getRequest<IRequestApp & { __apiKey: ApiKeyDoc }>();
