...
import { ApiKeyPublicProtected, GetApiKey } from 'src/common/api-key/decorators/api-key.decorator';
import {
...
export class ApiKeyAdminController {
  ...
  @Get('/list')
  async list(
    @PaginationQuery(API_KEY_DEFAULT_PER_PAGE, API_KEY_DEFAULT_ORDER_BY, API_KEY_DEFAULT_ORDER_DIRECTION, API_KEY_DEFAULT_AVAILABLE_SEARCH, API_KEY_DEFAULT_AVAILABLE_ORDER_BY)
    { _search, _limit, _offset, _order }: PaginationListDto,
    @PaginationQueryFilterInBoolean('isActive', API_KEY_DEFAULT_IS_ACTIVE) isActive: Record<string, any>,
    @PaginationQueryFilterInEnum('type', API_KEY_DEFAULT_TYPE, ENUM_API_KEY_TYPE) type: Record<string, any>,
  ): Promise<IResponsePaging> {
    const find: Record<string, any> = { ..._search, ...isActive, ...type };

    const apiKeys: ApiKeyEntity[] = await this.apiKeyService.findAll<ApiKeyEntity>(find, {
    ...
    return { _pagination: { totalPage, total }, data: apiKeys };
  }
  ...
  async create(@Body() body: ApiKeyCreateDto): Promise<IResponse> {
    ...
    return { data: { _id: created.doc._id, secret: created.secret } };
  }
  ...
  async reset(@GetApiKey() apiKey: ApiKeyDoc): Promise<IResponse> {
    ...
    return { data: { _id: updated._id, secret } };
  }
