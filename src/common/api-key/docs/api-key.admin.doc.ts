...
import { ApiKeyDocParamsId, ApiKeyDocQueryIsActive } from 'src/common/api-key/constants/api-key.doc.constant';
...
import { Doc, DocAuth, DocDefault, DocErrorGroup, DocOneOf, DocRequest, DocGuard, DocResponse, DocResponsePaging } from 'src/common/doc/decorators/doc.decorator';
...
export function ApiKeyAdminListDoc(): MethodDecorator {
  return applyDecorators(
    ...
    DocResponsePaging<ApiKeyListSerialization>('apiKey.list', { serialization: ApiKeyListSerialization }),
  );
}
...
export function ApiKeyAdminResetDoc(): MethodDecorator {
  return applyDecorators(
    ...
    DocAuth({ apiKey: true, jwtAccessToken: true }),
    DocGuard({ role: true, policy: true }),
    DocResponse<ApiKeyResetSerialization>('apiKey.reset', { serialization: ApiKeyResetSerialization }),
    DocErrorGroup([
      DocDefault({
        httpStatus: HttpStatus.NOT_FOUND,
        statusCode: ENUM_API_KEY_STATUS_CODE_ERROR.API_KEY_NOT_FOUND_ERROR,
...
export function ApiKeyAdminUpdateDoc(): MethodDecorator {
  return applyDecorators(
    ...
    DocAuth({ apiKey: true, jwtAccessToken: true }),
    DocGuard({ role: true, policy: true }),
    DocResponse<ResponseIdSerialization>('apiKey.update', { serialization: ResponseIdSerialization }),
    DocErrorGroup([
      DocDefault({
        httpStatus: HttpStatus.NOT_FOUND,
        statusCode: ENUM_API_KEY_STATUS_CODE_ERROR.API_KEY_NOT_FOUND_ERROR,
