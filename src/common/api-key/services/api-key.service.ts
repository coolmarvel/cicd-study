...
import { ApiKeyUpdateDateDto, ApiKeyUpdateDto } from 'src/common/api-key/dtos/api-key.update.dto';
...
export class ApiKeyService implements IApiKeyService {
  ...
  async findAll<T = ApiKeyDoc>(find?: Record<string, any>, options?: IDatabaseFindAllOptions): Promise<T[]> {
  ...
  async create({ name, type, startDate, endDate }: ApiKeyCreateDto, options?: IDatabaseCreateOptions): Promise<IApiKeyCreated> {
  ...
  async createRaw({ name, key, type, secret, startDate, endDate }: ApiKeyCreateRawDto, options?: IDatabaseCreateOptions): Promise<IApiKeyCreated> {
    const hash: string = await this.createHashApiKey(key, secret);
  ...
  async active(repository: ApiKeyDoc, options?: IDatabaseSaveOptions): Promise<ApiKeyDoc> {
    repository.isActive = true;

    return this.apiKeyRepository.save(repository, options);
  }

  async inactive(repository: ApiKeyDoc, options?: IDatabaseSaveOptions): Promise<ApiKeyDoc> {
    repository.isActive = false;

    return this.apiKeyRepository.save(repository, options);
  }

  async update(repository: ApiKeyDoc, { name }: ApiKeyUpdateDto, options?: IDatabaseSaveOptions): Promise<ApiKeyDoc> {
    repository.name = name;

    return this.apiKeyRepository.save(repository, options);
  }

  async updateDate(repository: ApiKeyDoc, { startDate, endDate }: ApiKeyUpdateDateDto, options?: IDatabaseSaveOptions): Promise<ApiKeyDoc> {
  ...
  async reset(repository: ApiKeyDoc, secret: string, options?: IDatabaseSaveOptions): Promise<ApiKeyDoc> {
  ...
  async delete(repository: ApiKeyDoc, options?: IDatabaseSaveOptions): Promise<ApiKeyDoc> {
  ...
  async createKey(): Promise<string> {
    const random: string = this.helperStringService.random(25, { safe: false, upperCase: true });
    return `${this.env}_${random}`;
  }

  async createSecret(): Promise<string> {
    return this.helperStringService.random(35, { safe: false, upperCase: true });
  }
  ...
  async inactiveManyByEndDate(options?: IDatabaseManyOptions): Promise<boolean> {
    return this.apiKeyRepository.updateMany({ endDate: { $lte: this.helperDateService.create() }, isActive: true }, { isActive: false }, options);
  }
}
