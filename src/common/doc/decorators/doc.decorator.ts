...
import { IDocAuthOptions, IDocDefaultOptions, IDocOfOptions, IDocResponseOptions } from 'src/common/doc/interfaces/doc.interface';
...
export function DocDefault<T>(options: IDocDefaultOptions): MethodDecorator {
  ...
  const schema: Record<string, any> = {
    ...
    properties: { message: { example: options.messagePath }, statusCode: { type: 'number', example: options.statusCode } },
  };
  ...
  return applyDecorators(ApiExtraModels(ResponseSerialization<T>), ApiResponse({ status: options.httpStatus, schema }), ...docs);
}
...
export function DocOneOf<T>(httpStatus: HttpStatus, ...documents: IDocOfOptions[]): MethodDecorator {
  ...
  for (const doc of documents) {
    const oneOfSchema: Record<string, any> = {
      allOf: [{ $ref: getSchemaPath(ResponseSerialization<T>) }],
      properties: {
        message: { example: doc.messagePath },
        statusCode: { type: 'number', example: doc.statusCode ?? HttpStatus.OK },
      },
    };
    ...
    if (doc.serialization) {
      ...
      oneOfSchema.properties = {
        ...oneOfSchema.properties,
        data: { $ref: getSchemaPath(doc.serialization) },
      };
    }
  ...
  return applyDecorators(ApiExtraModels(ResponseSerialization<T>), ApiResponse({ status: httpStatus, schema: { oneOf } }), ...docs);
}
...
export function DocAnyOf<T>(httpStatus: HttpStatus, ...documents: IDocOfOptions[]): MethodDecorator {
  ...
  return applyDecorators(ApiExtraModels(ResponseSerialization<T>), ApiResponse({ status: httpStatus, schema: { anyOf } }), ...docs);
}
...
export function DocAllOf<T>(httpStatus: HttpStatus, ...documents: IDocOfOptions[]): MethodDecorator {
  ...
  for (const doc of documents) {
    const allOfSchema: Record<string, any> = {
      allOf: [{ $ref: getSchemaPath(ResponseSerialization<T>) }],
      properties: { message: { example: doc.messagePath }, statusCode: { type: 'number', example: doc.statusCode ?? HttpStatus.OK } },
    };
  ...
  return applyDecorators(ApiExtraModels(ResponseSerialization<T>), ApiResponse({ status: httpStatus, schema: { allOf } }), ...docs);
}
...
export function DocResponsePaging<T = void>(messagePath: string, options: IDocResponsePagingOptions<T>): MethodDecorator {
  const docs: IDocDefaultOptions = {
    httpStatus: options?.httpStatus ?? HttpStatus.OK,
    messagePath,
    statusCode: options?.statusCode ?? options?.httpStatus ?? HttpStatus.OK,
    ...
    ApiResponse({
      ...
      schema: {
        ...
        properties: {
          ...
          data: {
            type: 'array',
            items: { $ref: getSchemaPath(docs.serialization) },
          },
        },
      },
    }),
...
export function DocResponseFile(options?: IDocResponseFileOptions): MethodDecorator {
  ...
  return applyDecorators(ApiProduces(options?.fileType ?? ENUM_FILE_MIME.CSV), ApiResponse({ status: httpStatus }));
}
