...
@Injectable()
export class FileExcelValidationPipe<T = any, R = any> implements PipeTransform {
  ...
  async validateExtract(value: IFileExtract<R>, classDtos: ClassConstructor<T>): Promise<IHelperFileRows<T>[]> {
    ...
    for (const [index, extract] of value.extracts.entries()) {
      ...
      const validator: ValidationError[] = await validate(dto as Record<string, any>);

      if (validator.length > 0) {
        errors.push({ row: index, sheetName: extract.sheetName, errors: validator });
      }

      dtos.push({ sheetName: extract.sheetName, data: dto });
    }
