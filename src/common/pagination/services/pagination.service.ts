...
export class PaginationService implements IPaginationService {
  ...
  perPage(perPage?: number): number {
    return perPage ? (perPage > PAGINATION_MAX_PER_PAGE ? PAGINATION_MAX_PER_PAGE : perPage) : PAGINATION_PER_PAGE;
  }

  order(orderByValue = PAGINATION_ORDER_BY, orderDirectionValue = PAGINATION_ORDER_DIRECTION, availableOrderBy = PAGINATION_AVAILABLE_ORDER_BY): IPaginationOrder {
    const orderBy: string = availableOrderBy.includes(orderByValue) ? orderByValue : PAGINATION_ORDER_BY;
  ...
  search(searchValue = '', availableSearch: string[]): Record<string, any> | undefined {
    ...
    return { $or: availableSearch.map((val) => ({ [val]: { $regex: new RegExp(searchValue), $options: 'i' } })) };
  }
  ...
  filterContain(field: string, filterValue: string): Record<string, { $regex: RegExp; $options: string }> {
    return { [field]: { $regex: new RegExp(filterValue), $options: 'i' } };
  }

  filterContainFullMatch(field: string, filterValue: string): Record<string, { $regex: RegExp; $options: string }> {
    return { [field]: { $regex: new RegExp(`\\b${filterValue}\\b`), $options: 'i' } };
  }
