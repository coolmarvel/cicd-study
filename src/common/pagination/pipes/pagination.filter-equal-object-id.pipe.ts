...
export function PaginationFilterEqualObjectIdPipe(field: string, raw: boolean): Type<PipeTransform> {
  ...
  class MixinPaginationFilterEqualObjectIdPipe implements PipeTransform {
    ...
    async transform(value: string): Promise<Record<string, Types.ObjectId | string>> {
      if (!value) {
        return undefined;
      }
      ...
      if (raw) {
        ...
        res = this.paginationService.filterEqual<Types.ObjectId | string>(field, finalValue);
      }
