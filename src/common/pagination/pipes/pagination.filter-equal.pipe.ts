...
export function PaginationFilterEqualPipe(field: string, raw: boolean, options?: IPaginationFilterStringEqualOptions): Type<PipeTransform> {
  ...
  class MixinPaginationFilterEqualPipe implements PipeTransform {
    ...
    async transform(value: string): Promise<Record<string, string | number>> {
      ...
      if (options?.isNumber) {
        finalValue = this.helperNumberService.check(value) ? Number.parseInt(value) : value;
      }

      let res: Record<string, any>;
      if (raw) {
        res = { [field]: finalValue };
      } else {
        res = this.paginationService.filterEqual<string | number>(field, finalValue);
      }
