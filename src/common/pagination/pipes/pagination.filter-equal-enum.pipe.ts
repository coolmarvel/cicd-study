...
export function PaginationFilterEqualEnumPipe<T>(field: string, defaultValue: T, defaultEnum: Record<string, any>, raw: boolean): Type<PipeTransform> {
  ...
  class MixinPaginationFilterInEnumPipe implements PipeTransform {
    ...
    async transform(value: string): Promise<Record<string, T>> {
      const finalValue: T = value ? defaultEnum[value.toUpperCase()] ?? defaultValue : defaultValue;
