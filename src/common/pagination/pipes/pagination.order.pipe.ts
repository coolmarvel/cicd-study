...
export function PaginationOrderPipe(defaultOrderBy: string, defaultOrderDirection: ENUM_PAGINATION_ORDER_DIRECTION_TYPE, availableOrderBy: string[]): Type<PipeTransform> {
  ...
  class MixinPaginationOrderPipe implements PipeTransform {
    ...
    async transform(value: Record<string, any>): Promise<Record<string, any>> {
      ...
      return { ...value, _order: order, _availableOrderBy: availableOrderBy, _availableOrderDirection: availableOrderDirection };
    }
  }
