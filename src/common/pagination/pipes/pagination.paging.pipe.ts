...
export function PaginationPagingPipe(defaultPerPage: number): Type<PipeTransform> {
  ...
  class MixinPaginationPagingPipe implements PipeTransform {
    ...
    async transform(value: Record<string, any>): Promise<Record<string, any>> {
      const page: number = this.paginationService.page(Number.parseInt(value?.page ?? 1));
      const perPage: number = this.paginationService.perPage(Number.parseInt(value?.perPage ?? defaultPerPage));
      const offset: number = this.paginationService.offset(page, perPage);

      this.request.__pagination = { ...this.request.__pagination, page, perPage };

      return { ...value, page, perPage, _limit: perPage, _offset: offset };
    }
  }
