...
export function PaginationFilterInBooleanPipe(field: string, defaultValue: boolean[], raw: boolean): Type<PipeTransform> {
  ...
  class MixinPaginationFilterInBooleanPipe implements PipeTransform {
    ...
    async transform(value: string): Promise<Record<string, { $in: boolean[] } | boolean[]>> {
      ...
      if (value) {
        finalValue = this.helperArrayService.unique(value.split(',').map((val: string) => val === 'true'));
      }
