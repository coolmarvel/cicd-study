...
export function PaginationFilterContainPipe(field: string, raw: boolean, options?: IPaginationFilterStringContainOptions): Type<PipeTransform> {
  ...
  class MixinPaginationFilterContainPipe implements PipeTransform {
    ...
    async transform(value: string): Promise<Record<string, { $regex: RegExp; $options: string } | string>> {
      if (!value) {
        return undefined;
      }
