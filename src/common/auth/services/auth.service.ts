...
import { IAuthPassword, IAuthPayloadOptions } from 'src/common/auth/interfaces/auth.interface';
...
export class AuthService implements IAuthService {
  ...
  constructor(
    ...
    this.maxPasswordAttempt = this.configService.get<number>('auth.password.maxAttempt');
  }

  async encryptAccessToken(payload: AuthAccessPayloadSerialization): Promise<string> {
    return this.helperEncryptionService.aes256Encrypt(payload, this.accessTokenEncryptKey, this.accessTokenEncryptIv);
  }

  async decryptAccessToken({ data }: Record<string, any>): Promise<AuthAccessPayloadSerialization> {
    return this.helperEncryptionService.aes256Decrypt(data, this.accessTokenEncryptKey, this.accessTokenEncryptIv) as AuthAccessPayloadSerialization;
  }

  async createAccessToken(payloadHashed: string | AuthAccessPayloadSerialization): Promise<string> {
  ...
  async payloadAccessToken(token: string): Promise<AuthAccessPayloadSerialization> {
    return this.helperEncryptionService.jwtDecrypt(token) as AuthAccessPayloadSerialization;
  }

  async encryptRefreshToken(payload: AuthRefreshPayloadSerialization): Promise<string> {
    return this.helperEncryptionService.aes256Encrypt(payload, this.refreshTokenEncryptKey, this.refreshTokenEncryptIv);
  }

  async decryptRefreshToken({ data }: Record<string, any>): Promise<AuthRefreshPayloadSerialization> {
    return this.helperEncryptionService.aes256Decrypt(data, this.refreshTokenEncryptKey, this.refreshTokenEncryptIv) as AuthRefreshPayloadSerialization;
  }

  async createRefreshToken(payloadHashed: string | AuthRefreshPayloadSerialization): Promise<string> {
    return this.helperEncryptionService.jwtEncrypt(
      { data: payloadHashed },
      {
  ...
  async payloadRefreshToken(token: string): Promise<AuthRefreshPayloadSerialization> {
    return this.helperEncryptionService.jwtDecrypt(token) as AuthRefreshPayloadSerialization;
  }
  ...
  async createPayloadAccessToken(user: Record<string, any>, { loginFrom, loginWith, loginDate }: IAuthPayloadOptions): Promise<AuthAccessPayloadSerialization> {
    return { user, loginFrom, loginWith, loginDate };
  }

  async createPayloadRefreshToken(_id: string, { loginFrom, loginWith, loginDate }: AuthAccessPayloadSerialization): Promise<AuthRefreshPayloadSerialization> {
    return { user: { _id }, loginFrom, loginWith, loginDate };
  }
  ...
  async googleGetTokenInfo(accessToken: string): Promise<IHelperGooglePayload> {
