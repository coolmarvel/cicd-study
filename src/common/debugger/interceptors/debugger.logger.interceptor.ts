...
export class DebuggerInterceptor implements NestInterceptor<Promise<any>> {
  ...
  constructor(
    ...
    this.writeIntoFile = this.configService.get<boolean>('debugger.writeIntoFile');
  }
    ...
    if (context.getType() === 'http') {
      ...
      return next.handle().pipe(
        tap(() => {
          ...
          if (this.writeIntoFile) {
            this.debuggerService.info({ type: 'response', statusCode: response.statusCode, message: response.statusMessage });
          }
        }),
      );
    }
