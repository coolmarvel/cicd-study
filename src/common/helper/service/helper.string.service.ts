...
import { IHelperStringCurrencyOptions, IHelperStringPasswordOptions, IHelperStringRandomOptions } from 'src/common/helper/interface/helper.interface';
import { IHelperStringService } from 'src/common/helper/interface/helper.string-service.interface';

@Injectable()
export class HelperStringService implements IHelperStringService {
  randomReference(length: number): string {
    const timestamp = `${new Date().getTime()}`;
    const randomString: string = this.random(length, { safe: true, upperCase: true });

    return `${timestamp}${randomString}`;
  }

  random(length: number, options?: IHelperStringRandomOptions): string {
    const rString = options?.safe ? new RandExp(`[A-Z]{${length},${length}}`) : new RandExp(`\\w{${length},${length}}`);
  ...
  checkPasswordWeak(password: string, options?: IHelperStringPasswordOptions): boolean {
  ...
  checkPasswordMedium(password: string, options?: IHelperStringPasswordOptions): boolean {
    const length = options?.length ?? 6;
  ...
  checkPasswordStrong(password: string, options?: IHelperStringPasswordOptions): boolean {
    const length = options?.length ?? 6;
    const regex = new RegExp(`^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{${length},}$`);
