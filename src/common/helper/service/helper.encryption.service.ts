...
import { IHelperEncryptionService } from 'src/common/helper/interface/helper.encryption-service.interface';
...
export class HelperEncryptionService implements IHelperEncryptionService {
  ...
  aes256Encrypt(data: string | Record<string, any> | Record<string, any>[], key: string, iv: string): string {
    ...
    const cipher = AES.encrypt(JSON.stringify(data), key, { mode: mode.CBC, padding: pad.Pkcs7, iv: cIv });
  ...
  aes256Decrypt(encrypted: string, key: string, iv: string): string | Record<string, any> | Record<string, any>[] {
    const cIv = enc.Utf8.parse(iv);
    const cipher = AES.decrypt(encrypted, key, { mode: mode.CBC, padding: pad.Pkcs7, iv: cIv });
