...
import { ENUM_HELPER_FILE_EXCEL_TYPE } from 'src/common/helper/constant/helper.enum.constant';
import { IHelperFileService } from 'src/common/helper/interface/helper.file-service.interface';
import { IHelperFileRows, IHelperFileReadOptions } from 'src/common/helper/interface/helper.interface';
...
export class HelperFileService implements IHelperFileService {
  ...
  writeExcel<T = any>(rows: IHelperFileRows<T>[], options?: IHelperFileReadOptions): Buffer {
    // workbook
  ...
  readCsv(file: Buffer): IHelperFileRows {
    ...
    return { data: rows, sheetName: worksheetsName };
  }

  readExcel(file: Buffer, options?: IHelperFileReadOptions): IHelperFileRows[] {
    // workbook
    const workbook = read(file, { type: 'buffer', password: options?.password });
    ...
    for (let i = 0; i < worksheetsName.length; i++) {
      ...
      sheets.push({ data: rows, sheetName: worksheetsName[i] });
    }
