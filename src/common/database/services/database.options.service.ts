...
export class DatabaseOptionsService implements IDatabaseOptionsService {
  ...
  createOptions(): MongooseModuleOptions {
    ...
    const options = this.configService.get<string>('database.options') ? `?${this.configService.get<string>('database.options')}` : '';
    ...
    const mongooseOptions: MongooseModuleOptions = { uri, autoCreate: true, ...timeoutOptions };

    if (user && password) {
      mongooseOptions.auth = { username: user, password: password };
    }
