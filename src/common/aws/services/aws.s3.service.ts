...
import { IAwsS3PutItem, IAwsS3PutItemOptions, IAwsS3RandomFilename } from 'src/common/aws/interfaces/aws.interface';
...
export class AwsS3Service implements IAwsS3Service {
  ...
  async checkBucketExistence(): Promise<HeadBucketCommandOutput> {
    const command: HeadBucketCommand = new HeadBucketCommand({ Bucket: this.bucket });
  ...
  async listBucket(): Promise<string[]> {
    ...
    try {
      const listBucket: ListBucketsOutput = await this.s3Client.send<ListBucketsCommandInput, ListBucketsCommandOutput>(command);
  ...
  async listItemInBucket(prefix?: string): Promise<AwsS3Serialization[]> {
    const command: ListObjectsV2Command = new ListObjectsV2Command({ Bucket: this.bucket, Prefix: prefix });

    try {
      const listItems: ListObjectsV2Output = await this.s3Client.send<ListObjectsV2CommandInput, ListObjectsV2CommandOutput>(command);
  ...
  async getItemInBucket(pathWithFilename: string): Promise<Readable | ReadableStream<any> | Blob> {
    const command: GetObjectCommand = new GetObjectCommand({ Bucket: this.bucket, Key: pathWithFilename });
  ...
  async putItemInBucket(file: IAwsS3PutItem, options?: IAwsS3PutItemOptions): Promise<AwsS3Serialization> {
    ...
    const command: PutObjectCommand = new PutObjectCommand({ Bucket: this.bucket, Key: key, Body: content, ACL: acl });
  ...
  async deleteItemInBucket(pathWithFilename: string): Promise<void> {
    const command: DeleteObjectCommand = new DeleteObjectCommand({ Bucket: this.bucket, Key: pathWithFilename });
  ...
  async deleteItemsInBucket(pathWithFilename: string[]): Promise<void> {
    const keys: ObjectIdentifier[] = pathWithFilename.map((val: string) => ({ Key: val }));
    const command: DeleteObjectsCommand = new DeleteObjectsCommand({ Bucket: this.bucket, Delete: { Objects: keys } });
  ...
  async deleteFolder(dir: string): Promise<void> {
    const commandList: ListObjectsV2Command = new ListObjectsV2Command({ Bucket: this.bucket, Prefix: dir });
    ...
    try {
      ...
      const commandDeleteItems: DeleteObjectsCommand = new DeleteObjectsCommand({ Bucket: this.bucket, Delete: { Objects: listItems } });

      await this.s3Client.send<DeleteObjectsCommandInput, DeleteObjectsCommandOutput>(commandDeleteItems);

      const commandDelete: DeleteObjectCommand = new DeleteObjectCommand({ Bucket: this.bucket, Key: dir });
  ...
  async createMultiPart(file: IAwsS3PutItem, maxPartNumber: number, options?: IAwsS3PutItemOptions): Promise<AwsS3MultipartSerialization> {
    let path: string = options?.path ?? '/';
    path = path.startsWith('/') ? path.replace('/', '') : path;
    const acl: ObjectCannedACL = options?.acl ? (options.acl as ObjectCannedACL) : ObjectCannedACL.public_read;
    ...
    const filename = options?.customFilename ? `${options?.customFilename}.${mime}` : file.originalname;
    const key: string = path ? `${path}/${filename}` : filename;
    const multiPartCommand: CreateMultipartUploadCommand = new CreateMultipartUploadCommand({ Bucket: this.bucket, Key: key, ACL: acl });

    try {
      const response = await this.s3Client.send<CreateMultipartUploadCommandInput, CreateMultipartUploadCommandOutput>(multiPartCommand);
  ...
  async uploadPart(multipart: AwsS3MultipartSerialization, partNumber: number, content: string | Uint8Array | Buffer): Promise<AwsS3MultipartPartsSerialization> {
    ...
    try {
      ...
      return { eTag: ETag, partNumber: partNumber, size: content.length };
    } catch (err: any) {
      throw err;
    }
  }

  async updateMultiPart({ size, parts, ...others }: AwsS3MultipartSerialization, part: AwsS3MultipartPartsSerialization): Promise<AwsS3MultipartSerialization> {
    parts.push(part);

    return { ...others, size: size + part.size, lastPartNumber: part.partNumber, parts };
  }

  async completeMultipart(multipart: AwsS3MultipartSerialization): Promise<void> {
    const completeMultipartCommand: CompleteMultipartUploadCommand = new CompleteMultipartUploadCommand({
      Bucket: this.bucket,
      Key: multipart.path,
      UploadId: multipart.uploadId,
      MultipartUpload: { Parts: multipart.parts as CompletedPart[] },
    });

    try {
      await this.s3Client.send<CompleteMultipartUploadCommandInput, CompleteMultipartUploadCommandOutput>(completeMultipartCommand);
  ...
  async abortMultipart(multipart: AwsS3MultipartSerialization): Promise<void> {
    ...
    try {
      await this.s3Client.send<AbortMultipartUploadCommandInput, AbortMultipartUploadCommandOutput>(abortMultipartCommand);
  ...
  async createRandomFilename(path?: string): Promise<IAwsS3RandomFilename> {
    ...
    return { path: path ?? '/', customFilename: filename };
  }
}
