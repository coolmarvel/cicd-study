...
import { IPolicyAbility, IPolicyRule, PolicyHandler } from 'src/common/policy/interfaces/policy.interface';
...
export class PolicyGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    const policyRule = this.reflector.get<IPolicyRule[]>(POLICY_RULE_META_KEY, context.getHandler()) || [];

    const { user } = context.switchToHttp().getRequest<IRequestApp>();
    const { type, permissions } = user.user;

    const ability = this.policyAbilityFactory.defineAbilityFromRole({ type, permissions });

    const policyHandler: PolicyHandler[] = this.policyAbilityFactory.handlerRules(policyRule);
