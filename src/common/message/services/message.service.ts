...
import { IErrors, IErrorsImport, IErrorValidationImport } from 'src/common/error/interfaces/error.interface';
...
export class MessageService implements IMessageService {
  ...
  constructor(
    ...
    this.appDefaultAvailableLanguage = this.configService.get<string[]>('message.availableLanguage');
  }
  ...
  filterLanguage(customLanguages: string[]): string[] {
    return this.helperArrayService.getIntersection(customLanguages, this.appDefaultAvailableLanguage);
  }

  setMessage(lang: string, key: string, options?: IMessageSetOptions): string {
    return this.i18n.translate(key, { lang: lang, args: options?.properties }) as any;
  }
      ...
      for (const constraint of constraints) {
        errors.push({
          ...
          message: this.get(`request.${constraint}`, {
            ...
            properties: { property, value: propertyValue },
          }),
        });
      }
  ...
  getImportErrorsMessage(errors: IErrorValidationImport[], options?: IMessageErrorOptions): IErrorsImport[] {
  ...
  get<T = string>(key: string, options?: IMessageOptions): T {
    const customLanguages = options?.customLanguages?.length > 0 ? this.filterLanguage(options.customLanguages) : [this.appDefaultLanguage];

    if (customLanguages.length > 1) {
      return customLanguages.reduce((a, v) => ({ ...a, [v]: this.setMessage(v, key, { properties: options?.properties }) }), {}) as any;
    }

    return this.setMessage(customLanguages[0], key, { properties: options?.properties }) as any;
  }
}
